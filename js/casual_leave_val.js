function casual_leave_val()
{
	var from_date = document.casual_leave.from_date.value;
	var date_pat = /^(0?[1-9]|[12][0-9]|3[01])[\/](0?[1-9]|1[012])[\/](0?\d{4})$/;

	if(!(date_pat.test(from_date)))
	{
		alert("Enter Valid From Date");
		document.casual_leave.from_date.focus();
		return false;
	}
	...
	var to_date = document.casual_leave.to_date.value;

	if(!(date_pat.test(to_date)))
	{
		alert("Enter Valid To Date");
		document.casual_leave.to_date.focus();
		return false;
	}
	...
	var leave_reason = document.casual_leave.leave_reason.value;

	if(leave_reason == "")
	{
		alert("Enter Valid Reason");
		document.casual_leave.leave_reason.focus();
		return false;
	}
