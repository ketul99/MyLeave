function work_leave_val()
{
	var from_date = document.work_leave.from_date.value;
	var date_pat = /^(0?[1-9]|[12][0-9]|3[01])[\/](0?[1-9]|1[012])[\/](0?\d{4})$/;

	if(!(date_pat.test(from_date)))
	{
		alert("Enter Valid From Date");
		document.work_leave.from_date.focus();
		return false;
	}
	...
	var to_date = document.work_leave.to_date.value;

	if(!(date_pat.test(to_date)))
	{
		alert("Enter Valid To Date");
		document.work_leave.to_date.focus();
		return false;
	}
	...
	var leave_reason = document.work_leave.leave_reason.value;

	if(leave_reason == "")
	{
		alert("Enter Valid Reason");
		document.work_leave.leave_reason.focus();
		return false;
	}

	var file = document.work_leave.fileToUpload.value;

	if(file == "")
	{
		alert("Select Valid Document");
		document.work_leave.fileToUpload.focus();
		return false;
	}
}