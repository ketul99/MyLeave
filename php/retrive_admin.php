<?php
	...
	$select_query = "select * from request_leave where status like 'pending'";
	...
	while ($row = mysqli_fetch_array($result))
	{
		...
		echo '<button type="submit" name="accept" formaction="../php/remark.php?leave_id=' . $row['leave_id'] . '&status=Accepted" class="btn btn-success" style="font-family: Comic Sans MS; margin-right: 5px;">Accept</button>';
		echo '<button type="submit" name="reject" formaction="../php/remark.php?leave_id=' . $row['leave_id'] . '&status=Rejected" class="btn btn-danger" style="font-family: Comic Sans MS;">Reject</button>';
		echo '</div>';
		echo '</form>';
		echo '</div>';
		echo '</div>';
