	...
	<div class="row">
		...
		<div class="col-sm-10">
			<?php
				...
				$select_query = "select * from request_late_entry where status like 'pending'";
				...
				while ($row = mysqli_fetch_array($result))
				{
					...
					echo '</div>';
					echo '<div class="col-sm-6">';
					echo '<form name="remark" method="POST" >';
					echo  '<div class="form-group input-group">';
					echo '<input type="text" name="remark" placeholder="Remark" class="form-control" style="margin-right: 5px;">';
					echo '<button type="submit" name="accept" formaction="../php/remark_late_entry.php?late_entry_id=' . $row['late_entry_id'] . '&status=Accepted" class="btn btn-success" style="font-family: Comic Sans MS; margin-right: 5px;">Accept</button>';
					echo '<button type="submit" name="reject" formaction="../php/remark_late_entry.php?late_entry_id=' . $row['late_entry_id'] . '&status=Rejected" class="btn btn-danger" style="font-family: Comic Sans MS;">Reject</button>';
					echo '</div>';
					echo '</div>';
					echo '</div>';
					echo '</div>';
